     ...
     if (searchTerm && typeof searchTerm === 'string' && searchTerm.trim()) {
          ...
          const matchingUserIds = await User.find({
               $or: [
                    ...
                    { phoneNumber: { $regex: trimmedSearchTerm, $options: 'i' } },
               ],
          }).distinct('_id');
     ...
     try {
          ...
          const result = await Subscription.find(whereConditions)
               .populate([
                    ...
                    {
                         path: 'userId',
                         select: 'email name linkedIn phoneNumber company website',
                    },
               ])
...
const createSubscriptionCheckoutSession = async (userId: string, packageId: string) => {
     ...
     if (!user?.stripeCustomerId) {
          // Create Stripe customer using user email
          ...
          await user.save(); // or your ORM’s update method
     }
